...
const StatusNavbar = () => {
  ...
  const getLinkStyle = (linkStatus: string) => {
    const baseStyle =
      "flex justify-center items-center gap-2 w-[13.4rem] h-12 hover:bg-blue-100/35 transition-colors duration-500 border-b-2";
    return currentStatus === linkStatus.toLowerCase()
      ? `${baseStyle} bg-blue-100/65 font-semibold border-b-2 border-b-blue-800`
      : baseStyle;
  };
